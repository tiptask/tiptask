...
export default function RequestsPage() {
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel(`requests-live-${sessionId}`)
      .on('postgres_changes', { event: '*', schema: 'public', table: 'task_requests', filter: `session_id=eq.${sessionId}` },
  ...
  const Card = ({ req, showActions }: { req: any, showActions: boolean }) => {
    ...
    return (
      <div className={`rounded-xl border p-3 transition ${showActions ? 'border-[#4AFFD4]/20 bg-[#4AFFD4]/[0.03]' : 'border-white/[0.06] bg-[#111117]'}`}>
        <div className="flex items-start justify-between gap-2 mb-1.5">
          ...
          <div className="text-right shrink-0">
            ...
            {expiry && <p className={`text-xs ${isExpiringSoon ? 'text-red-400' : 'text-white/25'}`}>⏱ {expiry}</p>}
          </div>
        </div>
        {req.message && <p className="text-white/35 text-xs italic mb-2">"{req.message}"</p>}
        <div className="text-white/15 text-xs mb-2">Platform fee: {req.platform_fee || (req.amount * commissionRate).toFixed(2)} {currency} ({Math.round(commissionRate*100)}%)</div>
        {showActions && (
        ...
        <div className="max-w-xl mx-auto p-4">
          ...
          {!session ? (
            ...
            <div className="space-y-5">
              <div>
                <div className="flex items-center gap-2 mb-2"><h2 className="text-white/50 text-xs font-semibold uppercase tracking-widest">Pending</h2>{pending.length > 0 && <span className="bg-amber-500/20 text-amber-400 text-xs px-1.5 py-0.5 rounded-full font-bold">{pending.length}</span>}</div>
                {pending.length === 0 ? <p className="text-white/20 text-xs">No pending requests</p> : <div className="space-y-2">{pending.map(r => <Card key={r.id} req={r} showActions={true} />)}</div>}
              </div>
              {accepted.length > 0 && (
                <div>
              ...
              {done.length > 0 && (
                <div>
                  <h2 className="text-white/50 text-xs font-semibold uppercase tracking-widest mb-2">Completed</h2>
