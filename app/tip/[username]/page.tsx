...
// Minimum amounts equivalent to ~1 USD
...
export default function TipPage({ params: paramsPromise }: { params: Promise<{ username: string }> }) {
  ...
  async function handleSubmit() {
    ...
    // Determine effective minimum for this mode
    ...
    if (activeMode === 'tip') {
      ...
      // If task has a fixed price, enforce exact amount
    ...
    <main className="min-h-screen bg-[#0a0a0a] text-white p-6">
      <div className="max-w-md mx-auto pt-8">
        <div className="text-center mb-8">
          <p className="text-white/40 text-sm mb-1">{creator.display_name}</p>
        ...
        <button onClick={() => setClientSecret(null)}
          className="w-full text-white/30 text-sm py-4 hover:text-white/60 transition mt-2">
          ← Go back
        </button>
      </div>
    </main>
  ...
  return (
    <main className="min-h-screen bg-[#0a0a0a] text-white pb-40">
      {/* Header */}
      <div className="pt-10 pb-6 px-6 text-center border-b border-white/5">
        <div className="w-14 h-14 rounded-full bg-white/10 flex items-center justify-center mx-auto mb-3 text-xl font-bold">
        ...
        <div className="flex items-center justify-center gap-3 mt-1">
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-green-400 text-xs">Live now</span>
          </div>
          {name && session && (
            <button onClick={() => setShowHistory(true)}
              className="text-white/30 text-xs hover:text-white/60 transition underline">
              My requests
            </button>
          )}
        </div>
      ...
      <div className="max-w-md mx-auto px-4 pt-6 space-y-3">
        ...
        {session.show_tasks && tasks.length > 0 && (
          <div className={`rounded-2xl border transition-all duration-200 overflow-hidden ${
            activeMode === 'task' ? 'border-white/20 bg-white/5' : 'border-white/8 bg-white/[0.03]'
          }`}>
            <button
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <div className="text-left">
                  <p className="font-semibold text-sm">Request a task</p>
                  <p className="text-white/40 text-xs mt-0.5">{tasks.length} tasks available</p>
                </div>
              </div>
              <span className={`text-white/30 transition-transform duration-200 ${activeMode === 'task' ? 'rotate-180' : ''}`}>▾</span>
            </button>
                    ...
                    <button key={task.id} onClick={() => selectTask(task)}
                      className={`w-full rounded-xl p-3.5 text-left transition-all ${
                        selectedTask?.id === task.id
                          ? 'bg-white text-black'
                          : 'bg-white/5 hover:bg-white/8 text-white'
                      }`}>
                      <div className="flex justify-between items-center">
                        <span className="font-medium text-sm">{task.title}</span>
                        <span className={`text-xs ${selectedTask?.id === task.id ? 'text-black/50' : 'text-white/40'}`}>
                      ...
                      {task.description && (
                        <p className={`text-xs mt-1 ${selectedTask?.id === task.id ? 'text-black/50' : 'text-white/30'}`}>
                          {task.description}
                        </p>
                      )}
                    </button>
                ...
                {selectedTask && (
                  <div className="space-y-3 pt-1">
                    <input type="text" value={name} onChange={e => setName(e.target.value.slice(0, 30))}
                      ...
                      className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-sm placeholder-white/20 focus:outline-none focus:border-white/30 transition" />
                    <input type="number" value={amount} onChange={e => setAmount(e.target.value)}
                      ...
                      className={`w-full border rounded-xl px-4 py-3 text-sm placeholder-white/20 focus:outline-none focus:border-white/30 transition ${
                        selectedTask.suggested_amount ? 'bg-white/3 border-white/5 cursor-not-allowed text-white/50' : 'bg-white/5 border-white/10'
                      }`} />
        ...
        {/* SECTION 2: Custom task */}
        {session.allow_custom_tasks && (
          <div className={`rounded-2xl border transition-all duration-200 overflow-hidden ${
            activeMode === 'custom' ? 'border-white/20 bg-white/5' : 'border-white/8 bg-white/[0.03]'
          }`}>
            <button
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <div className="text-left">
                  <p className="font-semibold text-sm">Custom request</p>
                  <p className="text-white/40 text-xs mt-0.5">Describe anything you want · min {globalMin} {currency}</p>
                </div>
              </div>
              <span className={`text-white/30 transition-transform duration-200 ${activeMode === 'custom' ? 'rotate-180' : ''}`}>▾</span>
            </button>
            ...
            {activeMode === 'custom' && (
              <div className="px-5 pb-5 space-y-3">
                <textarea value={customTask} onChange={e => setCustomTask(e.target.value.slice(0, 200))}
                  ...
                  rows={3}
                  className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-sm placeholder-white/20 focus:outline-none focus:border-white/30 transition resize-none" />
                <input type="text" value={name} onChange={e => setName(e.target.value.slice(0, 30))}
                  placeholder="Your name"
                  className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-sm placeholder-white/20 focus:outline-none focus:border-white/30 transition" />
                <input type="number" value={amount} onChange={e => setAmount(e.target.value)}
        ...
        {session.allow_free_tips && (
          <div className={`rounded-2xl border transition-all duration-200 overflow-hidden ${
            activeMode === 'tip' ? 'border-white/20 bg-white/5' : 'border-white/8 bg-white/[0.03]'
          }`}>
            <button
            ...
            >
              <div className="flex items-center gap-3">
                <span className="text-xl">💸</span>
                <div className="text-left">
                  <p className="font-semibold text-sm">Send a tip</p>
                  <p className="text-white/40 text-xs mt-0.5">Support without a specific task · min {minTip} {currency}</p>
                </div>
              </div>
              <span className={`text-white/30 transition-transform duration-200 ${activeMode === 'tip' ? 'rotate-180' : ''}`}>▾</span>
            </button>
      ...
      <div className="fixed bottom-0 left-0 right-0 p-4 bg-gradient-to-t from-[#0a0a0a] via-[#0a0a0a] to-transparent pt-8">
        <div className="max-w-md mx-auto">
          <button onClick={handleSubmit} disabled={submitting || activeMode === null}
            className="w-full py-4 rounded-2xl font-semibold text-base transition-all disabled:opacity-30 bg-white text-black hover:bg-white/90 active:scale-[0.98]">
