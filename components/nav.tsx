          ...
          {!authReady ? (
            ...
            <>
              <Link href="/auth/login"
                className="px-3 py-1.5 rounded-xl text-sm font-medium text-white/40 hover:text-white/70 transition">
                Sign in
              </Link>
              <Link href="/auth/register"
                className="px-3 py-1.5 rounded-xl text-sm font-bold bg-[#4AFFD4] text-[#08080C] hover:bg-[#6FFFDF] transition">
                Join free
              </Link>
            </>
          )}
...
export function BackButton({ href, label = '← Back' }: { href?: string; label?: string }) {
  ...
  return (
    <button
      ...
      className="text-white/30 hover:text-white/60 transition text-sm flex items-center gap-1">
      {label}
    </button>
  )
}
